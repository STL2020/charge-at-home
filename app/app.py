...
PROJECT_STATUS = [
    ...
    {"sprint": 9, "id": "S9-19", "modul": "BMW Telematik", "text": "SCHWERWIEGENDER BUG BEHOBEN: _speichere_fahrzeugdaten() hat den gesamten Fahrzeugdaten-Datensatz bei JEDEM Abruf ERSETZT statt zu ERGAENZEN. Lieferte BMW bei einem Abruf nur einen Teil der Felder (durchaus ueblich), gingen zuvor bekannte, weiterhin gueltige Werte (Ladestand, Reichweite, Akkukapazitaet etc.) ersatzlos verloren -- das war die eigentliche Ursache, warum die Batterieanzeige im Dashboard nach kurzer Zeit wieder leer wurde. Jetzt wird auf den bestehenden Datensatz aufgesetzt, nur tatsaechlich gelieferte Felder werden ueberschrieben. Mit dem exakten Fehlerszenario (voller Abruf gefolgt von Teil-Abruf) nachgestellt und bestaetigt behoben.", "status": "fertig", "view": "dashboard"},
    {"sprint": 9, "id": "S9-20", "modul": "Dashboard", "text": "Fahrzeugstatus-Kachel grundlegend neu ausgerichtet an der Frage 'was will der Nutzer sehen': Reichweite/Ladestand (Balken), Ladestatus 'Angesteckt/Nicht angesteckt' (neuer Datenpunkt, live), HU/TUEV-, Service- und Bremsfluessigkeits-Termine mit Dringlichkeits-Einfaerbung (rot ueberfaellig/binnen 30 Tagen, gelb binnen 90 Tagen) direkt aus den Fahrzeug-Stammdaten -- vorher nirgends im Dashboard sichtbar, obwohl laengst im Fahrzeug-Datensatz vorhanden. Google-Maps-Iframe durch eine im Dunkelmodus stimmige Adresstext-Karte ersetzt (Rueckwaertsgeocoding wie bei Fahrten), mit Link zur Route statt eingebetteter, hell gestalteter Fremdkarte. Versionsangabe aus dem Dashboard-Kopf entfernt und stattdessen unten in die Seitenleiste verschoben, wo sie der neuen Uhr nicht mehr im Weg steht.", "status": "fertig", "view": "dashboard"},
    {"sprint": 9, "id": "S9-21", "modul": "BMW Telematik", "text": "URSACHE GEFUNDEN UND BEHOBEN, warum die Status-Kachel trotz aktivem Datenstrom (306 empfangene Meldungen) leer blieb: verarbeite_nachricht() im Stream-Handler hat AUSSCHLIESSLICH Breitengrad/Laengengrad/Kilometerstand fuer die Fahrterkennung ausgewertet -- alle anderen Datenpunkte derselben Nachricht (Reichweite, Akkukapazitaet, Verbrauch, Ladestand, Angesteckt-Status) wurden stillschweigend verworfen, obwohl sie nachweislich im Strom ankamen (siehe echtes Protokoll vom 27.08., 18:48 Uhr: kombiRemainingElectricRange=426, maxEnergy=81 mehrfach empfangen, aber nie gespeichert). Die Status-Kachel konnte sich dadurch nur ueber den separaten, taeglich limitierten manuellen Abruf aktualisieren, nie aus dem frei fliessenden Strom. Jetzt extrahiert der Stream-Handler bei jeder Nachricht zusaetzlich alle Fahrzeugdaten-Deskriptoren und legt sie ueber dieselbe (zusammenfuehrende) Speicherfunktion ab, die auch der manuelle Abruf nutzt. Mit Stephans exakten echten Protokollzeilen als Testfall nachgestellt und bestaetigt: Reichweite und Akkukapazitaet kommen jetzt korrekt aus dem Stream in der Kachel an.", "status": "fertig", "view": "dashboard"},
    {"sprint": 9, "id": "S9-22", "modul": "BMW Telematik", "text": "Ladestand (SoC) fehlt weiterhin in Stephans Datenstrom -- nachweislich in ZWEI unabhaengigen Protokoll-Auszuegen nicht enthalten (vehicle.powertrain.electric.battery.stateOfCharge.displayed kommt nie an), obwohl die uebrigen Werte (Reichweite, Akkukapazitaet, Verbrauch, km) zuverlaessig ankommen. Das ist keine Code-Einschraenkung, sondern ein Datenverfuegbarkeitsproblem auf BMW-Seite oder in der Portal-Konfiguration -- offen kommuniziert statt eine weitere 'Reparatur' vorzutaeuschen, die technisch nichts aendern koennte.", "status": "offen", "view": "dashboard"},
    {"sprint": 9, "id": "S9-23", "modul": "Geocoding", "text": "Rueckwaertssuche (Koordinaten zu Adresse) von einem einzelnen Dienst (Photon) auf zwei unabhaengige Dienste mit Rueckfallkette umgestellt: Photon zuerst, bei Fehlschlag OpenStreetMaps eigener Nominatim-Dienst -- beide kostenlos, kein API-Key, keine Registrierung. Betrifft sowohl Fahrten-Adressen als auch die neue Standort-Anzeige im Dashboard. Fallback-Kette mit simulierten Antworten getestet (echte Netzwerkverbindung zu beiden Diensten war aus der Entwicklungsumgebung heraus nicht pruefbar).", "status": "fertig", "view": None},
    {"sprint": 9, "id": "S9-24", "modul": "BMW Telematik", "text": "BUG BEHOBEN: stelle_container_sicher() verwendete einen einmal angelegten Container fuer immer weiter, ohne je zu pruefen, ob sich CONTAINER_DESCRIPTORS seitdem geaendert hat. Neu hinzugekommene Datenpunkte (maxEnergy, isPlugged, urspruenglich auch stateOfCharge) wurden dadurch von BMW nie ausgeliefert -- weder ueber den manuellen Abruf noch indirekt, weil der Container schlicht nicht danach gefragt hat. Jetzt vergleicht jeder Aufruf die gewuenschten Deskriptoren mit den beim Container hinterlegten; bei Abweichung wird der alte Container (best effort) geloescht und automatisch ein neuer mit der aktuellen Liste angelegt. Migration fuer bestehende Datenbanken (neue Spalte 'container_deskriptoren') und Erkennungslogik mit einem simulierten veralteten Container bestaetigt.", "status": "fertig", "view": "fahrzeuge"},
    {"sprint": 9, "id": "S9-25", "modul": "Dashboard", "text": "URSACHE fuer 'Stand bleibt eingefroren trotz Aktualisieren-Klicks' identifiziert: das taegliche Tageslimit (50 Abrufe) war vermutlich durch die intensiven Tests des Tages ausgeschoepft -- BMW antwortet dann mit CU-429, was bisher nur als fluechtige, leicht zu uebersehende Meldung erschien und dabei den nachfolgenden Kachel-Refresh unterband (Kontingent blieb dadurch unsichtbar, 'eingefroren' wirkend). Zwei Behebungen: (1) Kontingent-Stand ('X von 50 Abrufen heute uebrig' bzw. 'Tageslimit erreicht') jetzt dauerhaft in der Kachel sichtbar statt nur als Toast, Aktualisieren-Knopf wird bei erreichtem Limit sichtbar deaktiviert; (2) die Kachel rendert jetzt auch bei einem fehlgeschlagenen Abruf neu, statt bei einem Fehler stehen zu bleiben. Der Datenstrom (MQTT) ist vom Tageslimit unabhaengig und bleibt der bevorzugte Weg.", "status": "fertig", "view": "dashboard"},
    {"sprint": 9, "id": "S9-26", "modul": "BMW Telematik", "text": "Feld-fuer-Feld-Abgleich zwischen den 17 verwendeten CarData-Deskriptoren und ihrer tatsaechlichen Verarbeitung durchgefuehrt: trip.segment.end.* (3 Deskriptoren) werden nur im REST-Pfad gebunden, nicht im Stream-Handler (Stream nutzt fuer die Fahrterkennung stattdessen die eigene lat/lon/km-Pufferlogik); checkControlMessages wird von BMW angefordert, aber in keinem der beiden Pfade ausgewertet. Beide Befunde dokumentiert, keine Aenderung vorgenommen (auf ausdruecklichen Wunsch nur Analyse, keine Programmierung).", "status": "offen", "view": None},
    {"sprint": 9, "id": "S9-27", "modul": "Dashboard", "text": "Fahrzeugstatus-Kachel grundlegend umgebaut: zeigt jetzt IMMER ihren vollstaendigen Rahmen (Batteriebalken, Termine, Kennzahlen-Kacheln, Standort-Box), auch wenn einzelne oder alle Werte fehlen -- an die Stelle fehlender Werte tritt ein klar erkennbarer Leerstand ('—', 'nicht hinterlegt', 'Ladestand noch nicht bekannt') statt das jeweilige Element oder die halbe Kachel komplett zu verstecken. 'Angesteckt'-Status zeigt bei fehlendem Wert standardmaessig 'Nicht angesteckt' statt gar nichts. Mit isoliertem Rendering-Test fuer den Leerfall (Fahrzeug ohne jemals empfangene Daten) UND den Vollfall bestaetigt -- keine 'undefined'/'NaN'-Reste, alle drei Termine-Zeilen und alle vier Kennzahlen-Kacheln erscheinen in beiden Faellen. Ausserdem: irrefuehrender Platzhaltertext im Reifengroesse-Feld ('245 40 R 19 98 Y' sah wie echte Daten aus) auf 'z. B. 245/40 R19' korrigiert.", "status": "fertig", "view": "dashboard"},
    ...
    {"sprint": 9, "id": "S9-38", "modul": "Layout", "text": "Nutzer wies zurecht auf eine grosse Luecke zwischen dem im Chat freigegebenen HTML-Mockup und der echten App hin: das Mockup war eine eigenstaendige Datei mit eigenem Layout-System, die neuen Bausteine wurden zwar uebernommen, das UMGEBENDE Layout der echten App (Seitenpolster, Tabellen-Zeilenhoehe) aber nie nachgezogen. Behoben: .content-Seitenpolster von 20px/34px auf 18px/22px verkleinert (Dashboard ruecht naeher an die Sidebar), Tabellen-Zeilenhoehe von der zuvor selbst zu grosszuegig gewaehlten 13px auf 9px zurueckgenommen. Wirkt global auf alle Seiten/Tabellen der App.", "status": "fertig", "view": None},
    {"sprint": 9, "id": "S9-39", "modul": "Dashboard", "text": "Standort-Klick oeffnet jetzt ein In-Seite-Popup mit eingebetteter OpenStreetMap-Karte (_standortPopupZeigen) statt eines neuen Browser-Tabs zu Google Maps. Pulsierender Standort-Punkt faerbt sich jetzt danach ein, ob das Fahrzeug zuhause oder unterwegs ist (gruen/blau), nicht mehr nach Ladestand -- neue oeffentliche Funktion geocoding_service.hole_heim_adresse() extrahiert die bereits bestehende Strassenvergleichs-Logik (Hausnummer bewusst ignoriert) aus einer privaten Funktion heraus, damit sie auch von der Status-Kachel genutzt werden kann, ohne sie zu duplizieren. Mit eigenem Test fuer beide Faelle (zuhause/unterwegs) bestaetigt. Wallbox-Statuspill oben im Dashboard ist jetzt klickbar und oeffnet die Wallboxen-Seite.", "status": "fertig", "view": "dashboard"},
    {"sprint": 9, "id": "S9-40", "modul": "Fahrzeuge", "text": "HU/TUEV, Service und Kilometerstand zeigten bei fehlendem Wert bisher eine Sackgasse ('nicht hinterlegt', kein Ausweg). Bearbeitbar waren sie tatsaechlich schon -- nur unter 'Fahrzeuge', von der Status-Kachel aus nicht erreichbar. Jetzt klickbarer Link direkt zur Bearbeitung (editVehicle), kein doppelter Eingabe-Weg noetig.", "status": "fertig", "view": "fahrzeuge"},
    {"sprint": 9, "id": "S9-41", "modul": "Fahrten", "text": "Die Distanz-Zeile im 'Neue Fahrt'-Formular quetschte Eingabefeld und zwei Buttons mit Textbeschriftung in eine einzige schmale Reihe -- erzwang horizontales Scrollen im Panel. Auf zwei Zeilen aufgeteilt: Distanz-Eingabe allein, Buttons darunter mit Umbruch bei Bedarf.", "status": "fertig", "view": "fahrten"},
    {"sprint": 9, "id": "S9-42", "modul": "Dashboard", "text": "Karussell-Auswahl fuer mehrere Fahrzeuge: bei 2+ verbundenen Fahrzeugen erscheinen kleine Reiterkarten (Name + Farbpunkt nach Lade-/Akkustatus) oberhalb eines horizontal einrastenden Scroll-Bereichs (CSS scroll-snap) -- Wischen auf Touch-Geraeten oder Reiter-Klick wechseln animiert zum naechsten Fahrzeug, statt alle Kacheln untereinander zu stapeln. Aktiver Reiter bleibt ueber Dashboard-Refreshs erhalten. Mit isoliertem Rendering-Test fuer drei simulierte Fahrzeuge bestaetigt (Reiter, Spuren, Farblogik, Navigationsfunktion).", "status": "fertig", "view": "dashboard"},
    {"sprint": 9, "id": "S9-43", "modul": "Layout", "text": "Echte Review-Sitzung mit Playwright (echter Browser, echte Screenshots der laufenden App) deckte zwei ernste, zuvor uebersehene Fehler auf, die reines Code-Lesen nicht gefunden haette. 1) Der 'Anchor left'-Fix aus S9-38 hatte nur das Innenpolster korrigiert, die eigentliche Ursache des grossen Leerraums (.content mit max-width:1680px UND margin-left:auto/margin-right:auto -- erzeugte auf breiten Bildschirmen 323px toten Raum links UND rechts durch Zentrierung statt Linksbuendigkeit) komplett uebersehen. Jetzt margin-left:0, mit Playwright bei 2560px Breite nachgemessen (marginLeft vorher 323px, nachher 0px, contentRect.left == Sidebar-Breite). 2) Karussell-Filter verlangte faelschlich eine ECHTE BMW-Anmeldung (status.angemeldet==true) fuer die Aufnahme ins Karussell -- ein selbst erfundenes, nie abgesprochenes Kriterium. Der Nutzer wollte: jedes neue Fahrzeug zaehlt, ob BMW-verbunden oder rein manuell angelegt. Filter entfernt, mit einem dritten, komplett manuellen Test-Fahrzeug (keine VIN) im Karussell bestaetigt.", "status": "fertig", "view": "dashboard"},
    ...
    {"sprint": 9, "id": "S9-48", "modul": "Fahrten", "text": "Detail-Formular quetschte die Tabelle daneben zusammen, anders als bei Ladevorgaenge wo dasselbe Nebeneinander gut funktioniert. Ursache war eine eigene frueherer Aenderung: .trips-detail hatte STARRE 620px (gegen 400px bei .sessions-detail), gesetzt um die interne Routenkarte unterzubringen -- die Tabelle bekam dadurch kaum Platz. Jetzt flexibel plus umbruchfaehiges internes Raster (auto-fit/minmax statt starrer '1fr 300px'): bei Platzmangel rutscht die Karte unter die Felder, statt beides zu quetschen.", "status": "fertig", "view": "fahrten"},
    {"sprint": 9, "id": "S9-49", "modul": "BMW Telematik", "text": "KERN-FEHLER, aus den MQTT-Protokollen des Nutzers diagnostiziert: BMW erlaubt nur EINE MQTT-Verbindung je GCID, die App oeffnete aber eine PRO FAHRZEUG. Teilen sich zwei Fahrzeuge ein BMW-Konto, warfen sie sich endlos gegenseitig hinaus -- in den Protokollen lueckenlos belegt durch ein exakt komplementaeres Muster (immer war genau eines verbunden, das andere bekam 'Quota exceeded', im Wechsel alle paar Minuten). Behoben: ein einziger Thread je GCID haelt die Verbindung und abonniert die Themen ALLER Fahrzeuge dieses Kontos; eingehende Nachrichten werden anhand des Themas (VIN) dem richtigen Fahrzeug zugeordnet. Zweitstart fuer ein Fahrzeug desselben Kontos oeffnet keine neue Verbindung mehr. Mit Test bestaetigt: zwei Fahrzeuge mit gleicher GCID erzeugen genau EINE Verbindung, die beide VINs bedient.", "status": "fertig", "view": "fahrzeuge"},
    {"sprint": 9, "id": "S9-50", "modul": "BMW Telematik", "text": "KORREKTUR eines eigenen Fehlers aus S9-28: Damals wurde keepalive von 60 auf 240 ERHOEHT, in der Annahme, selteneres Pingen entschaerfe die Abbrueche. Die Protokolle widerlegen das eindeutig -- danach riss die Verbindung exakt alle 60,0 Sekunden ab (vorher 60-70s), also schlimmer statt besser. Richtige Deutung: paho pingt erst nach rund 75 % des keepalive-Werts, bei 240s also nach ~180s, waehrend BMW bereits nach 60s ohne Verkehr trennt. Da ein geparktes Fahrzeug minutenlang nichts sendet, lief die Verbindung zwangslaeufig in diese Leerlauf-Grenze. Jetzt keepalive=30 (Ping etwa alle 22s), sicher innerhalb des 60s-Fensters. ZUSAETZLICH behoben: eine ABGELEHNTE Verbindung (falscher Token) erhoehte den Fehlschlag-Zaehler nicht, weil dieser nur bei echten Ausnahmen stieg -- die Zwangs-Token-Erneuerung nach zwei Fehlschlaegen griff daher nie, und die App versuchte es endlos mit demselben toten Token. Genau das zeigen die Protokolle ab 09:11 ('Bad user name or password' ueber zehn Minuten ohne Selbstheilung). Ausserdem nutzt die MQTT-Client-Kennung jetzt eine eigene, stabile ID statt der GCID selbst.", "status": "fertig", "view": "fahrzeuge"},
    {"sprint": 9, "id": "S9-51", "modul": "Protokoll", "text": "Navigationsfalle behoben: Beim Umschalten auf Quelle 'MQTT (roh)' wurde per positionsbasiertem Selektor die erste Karte der Ansicht ausgeblendet -- das war aber die FILTERLEISTE selbst. Der Anwender kam ohne Neuladen der Seite nicht mehr aus der Ansicht heraus. Filterleiste hat jetzt eine eigene ID und wird vom Selektor ausdruecklich ausgenommen.", "status": "fertig", "view": "protokoll"},
    {"sprint": 9, "id": "S9-52", "modul": "Fahrzeuge", "text": "Auf Hinweis des Nutzers: In der Oberflaeche fehlte jeder Hinweis darauf, WELCHE Datenpunkte im BMW-Portal aktiviert sein muessen -- ohne sie bleibt die Fahrzeugkachel leer, obwohl die Anmeldung funktioniert, und der Anwender kann die Ursache nicht erraten. Neue aufklappbare Checkliste direkt im Fahrzeug-Dialog, gegliedert nach Pflicht (Abrechnung/Fahrten), nur Elektro (Akku/Laden) und optional (Komfort), inklusive Hinweis fuer Verbrenner (nur der erste Block noetig) und Fehlersuche. Akku-/Ladefelder werden bei Antrieb 'Verbrenner' automatisch ausgeblendet.", "status": "fertig", "view": "fahrzeuge"},
    {"sprint": 9, "id": "S9-53", "modul": "Ladevorgaenge", "text": "Kennzahlen-Kacheln ergaenzt (geladene Energie, Kosten gesamt, Ø Preis je kWh, Aufteilung zuhause/unterwegs). Werden aus der FERTIG gefilterten Liste berechnet und passen dadurch immer exakt zu dem, was in der Tabelle darunter steht (Zeitraum, Wallbox, Quelle, 0-kWh-Schalter). EIGENER RECHENFEHLER beim Bauen gefunden und behoben: der Heimanteil der Kosten wurde zunaechst ueber den Durchschnittspreis ALLER Sessions hochgerechnet -- da unterwegs ein Vielfaches kostet (0,79 statt 0,34 €/kWh), verfaelschte das den erstattungsfaehigen Heimanteil massiv (9,60 € statt korrekt 4,87 €). Jetzt werden die Heimladekosten direkt aufsummiert.", "status": "fertig", "view": "ladesessions"},
    {"sprint": 9, "id": "S9-54", "modul": "Oberflaeche", "text": "Aufraeumarbeiten auf Nutzerwunsch: Detail-Formulare bei Fahrten UND Ladevorgaengen deutlich breiter (560px statt 400/620px, flexibel 420-680px) -- bei schmalem Panel wurden die Formulare so lang, dass der Speichern-Knopf aus dem sichtbaren Bereich rutschte. Knopf 'BMW CarData — Fahrten' aus der Fahrten-Ansicht entfernt: der Abruf laeuft ohnehin ueber den Datenstrom bzw. den Fahrzeug-Dialog, der zweite manuelle Knopf war eine Doppelung und verbrauchte stillschweigend vom knappen Tageskontingent (50 Abrufe).", "status": "fertig", "view": None},
    {"sprint": 9, "id": "S9-55", "modul": "Oberflaeche", "text": "Menuestruktur der Ladevorgaenge-Ansicht entruempelt: Die Aktionsleiste enthielt fuenf gleichrangige Knoepfe (Loxone-Log importieren, CSV importieren, Beispieldatei, Beleg Zeitraum, Alle loeschen) und wirkte dadurch unruhig. Die drei Import-Knoepfe sind samt Import-Panel in die Wallbox-Ansicht gewandert -- dort gehoeren sie fachlich hin, weil ein Import immer einer Wallbox zugeordnet wird. Beim Verschieben entstand eine doppelte Element-ID (import-result), durch einen Doppel-ID-Scan gefunden und entfernt. Die Erfolgsmeldung verweist per Link auf die Ladevorgaenge.", "status": "fertig", "view": "ladesessions"},
    {"sprint": 9, "id": "S9-56", "modul": "BMW Telematik", "text": "ERFOLG: Der Stream liefert nach den Korrekturen aus S9-49/S9-50 wieder Daten -- die gemeinsame Verbindung je GCID und der korrigierte keepalive wirken, im neuen Protokoll kommen ueber 20 Deskriptoren an. Die Auswertung dieses Protokolls deckte drei weitere echte Fehler auf. (1) STECKERTYP kam NIE zur Anzeige: die Zuordnungstabelle enthielt geratene Bezeichner ('AC_TYPE2PLUG'), das Fahrzeug sendet aber 'AC_TYP2COMBO' -- TYP statt TYPE, COMBO statt PLUG. Ersetzt durch eine Auswertung nach Bestandteilen (TYP1/TYPE1, TYP2/TYPE2, CCS/COMBO, AC/DC-Praefix), die gegen weitere Schreibweisen robust ist; mit allen bekannten Varianten getestet. (2) LADESTAND: der offizielle Deskriptor stateOfCharge.displayed kam in KEINEM Protokoll je an, obwohl im Portal aktiviert. Stattdessen liefert vehicle.drivetrain.batteryManagement.header zuverlaessig einen Prozentwert -- jetzt als RUECKFALL eingebaut (offizieller Wert behaelt Vorrang, Plausibilitaetspruefung 0-100). (3) VERRIEGELUNG: door.lock.status kommt bei diesem Fahrzeug ueberhaupt nicht, die Kachel behauptete aber trotzdem 'Verriegelt' -- eine frei erfundene Aussage ueber den Verschlusszustand eines Autos. Verriegelungs-, Ladeklappen- und Tuer-Chips erscheinen jetzt NUR bei tatsaechlich bekanntem Zustand.", "status": "fertig", "view": "fahrzeuge"},
    {"sprint": 9, "id": "S9-57", "modul": "Ladevorgaenge", "text": "Werkzeugleiste war weiterhin sichtbar schief: Zeitraum-Knoepfe und Feldfilter (Von/Bis/Wallbox/Quelle) lagen in EINER Reihe, obwohl die Feldgruppe wegen ihrer Beschriftungen deutlich hoeher ist. Jetzt zwei ruhige Zeilen: oben Zeitraum-Knoepfe plus Aktionen rechtsbuendig, darunter die Feldfilter.", "status": "fertig", "view": "ladesessions"},
    {"sprint": 9, "id": "S9-58", "modul": "Einstellungen", "text": "Katalog 'Anlaesse fuer Fahrten' vom Reiter 'Recht & Sicherheit' in den System-Reiter unter die neue Ueberschrift 'Kataloge' verschoben -- dort suchte ihn niemand, ein pflegbarer Nachschlage-Katalog gehoert fachlich zu System, nicht zu Rechtsthemen.", "status": "fertig", "view": "einstellungen"},
    {"sprint": 9, "id": "S9-59", "modul": "Dokumentation", "text": "Vollstaendige Feldzuordnungs-Tabelle in den BMW-Einstellungen ergaenzt: welcher BMW-Datenpunkt genau welches Feld/welchen Chip in der App fuellt, mit Kennzeichnung Pflicht/Elektro/optional und ausdruecklichem Hinweis auf die Paare, bei denen BEIDE Eintraege angehakt sein muessen (Akkukapazitaet, Angesteckt-Status). Steht bewusst zusaetzlich hier und nicht nur im Fahrzeug-Dialog -- wer in den Einstellungen nach der BMW-Anbindung sucht, erwartet die Erklaerung an dieser Stelle. Ausserdem die Checkliste im Fahrzeug-Dialog von einem zugeklappten, kleinen Aufklapper zu einem aufgeklappten, farblich hervorgehobenen Pflichthinweis gemacht -- der Nutzer meldete zu Recht, dass sie in der bisherigen Form uebersehen wurde, mit der Folge einer leeren Fahrzeugkachel ohne auffindbare Ursache.", "status": "fertig", "view": "einstellungen"},
    {"sprint": 9, "id": "S9-60", "modul": "Dashboard", "text": "Rueckmeldung 'Verriegelt keine Funktion': Der Chip verschwand seit S9-56 korrekt, wenn der Datenpunkt fehlt -- aber ohne jede Erklaerung, weshalb er schlicht kaputt wirkte. Statt still zu verschwinden erscheint jetzt ein gestrichelt umrandeter, anklickbarer Hinweis 'Verriegelung nicht aktiviert', der in die Einstellungen zur Feldzuordnung fuehrt. Fehlende Daten sind damit als solche erkennbar statt als Fehler der Anwendung.", "status": "fertig", "view": "dashboard"},
